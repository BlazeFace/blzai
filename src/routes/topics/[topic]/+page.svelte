...
<div class="container px-5 py-2 mx-auto lg:pt-12 lg:px-32">
        ...
        <div class="w-full p-1 md:p-2">
          {#if photoId !== selected}
            <div
              role="button"
              tabindex="0"
              aria-label="Expandable image"
              out:send={{ key: photoId }}
              in:receive={{ key: photoId }}
              on:click={() => handlePreviewClick(photoId)}
              on:keydown={() => handlePreviewClick(photoId)}
              class="image"
              style="background-image: url({`${cdn}preview_${photoId}.avif`});"
            ></div>
          {/if}
        </div>
      </div>
  ...
  {#if selected}
    ...
    >
      ...
      >
      <img
        alt="Feature to support caption for each is being worked on"
        in:receive={{ key: selected }}
        out:send={{ key: selected }}
        src={`${cdn}full_${selected}.avif`}
      />
    </div>
  {/if}
</div>
